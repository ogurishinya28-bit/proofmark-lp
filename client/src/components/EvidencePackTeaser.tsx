...
import { FileText, FileBadge, Workflow, Layers3, Mail, Link as LinkIcon, ArrowRight } from 'lucide-react';
import { PROOFMARK_COPY } from '../lib/proofmark-copy';

/**
 * ProofMark の差別化コアである "Evidence Pack" を、ホームに導入する
 * 1セクション分のティーザー。レポートの最終提言である
 * 「証明書発行 → 信用を納品できる状態にする」という体験を視覚化する。
 */
const ICONS = [FileText, FileBadge, Workflow, Layers3, Mail, LinkIcon];
          ...
          >
            {ev.items.map((item, index) => {
              ...
              return (
                <motion.li
                  key={item}
                  className="flex items-start gap-3 rounded-2xl border border-white/10 bg-[#0D0B24]/85 p-4 backdrop-blur-md"
                  variants={{
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 line-clamp-2 text-sm font-semibold text-white">{item}</div>
                  </div>
                </motion.li>
              );
            })}
